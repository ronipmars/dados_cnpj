...
--COPY (
SELECT concat(emp.cnpj_basico, est.cnpj_ordem , est.cnpj_dv) as cnpj
, emp.razao_social 
--, emp.natureza_juridica, nat.descricao
...
, concat(est.tipo_logradouro, ' ' , est.logradouro, ' - ', est.numero, ' - ', est.complemento, ' - ', est.bairro) as endereco
, est.cep, est.uf, munic.descricao
...
--	and socios.cnpj_basico = emp.cnpj_basico
	where 1=1
	and left(cnae.codigo,2) between '10' and '33'
	and nat.codigo between '2061' and '3100'
	and nat.codigo not in('2135')
	and est.uf = 'SP'
	--and est.cep between '06000000' and '09999999'
...
--	and cnae.codigo IN ( '6920601' -- contabilidades uf sp							)
	--limit 10
--	order by emp.cnpj_basico
--) TO 'D:\cnpj\export_lists\INDUSTRIAS_TRANSFORMACAO.csv' DELIMITER ';' CSV HEADER ENCODING 'latin1' ;
